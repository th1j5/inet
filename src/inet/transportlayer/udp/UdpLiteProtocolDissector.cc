...
void UdpLiteProtocolDissector::dissect(Packet *packet, const Protocol *protocol, ICallback& callback) const
{
    ...
    auto dataProtocol = ProtocolGroup::getUdpliteProtocolGroup()->findProtocol(header->getDestPort());
    if (dataProtocol == nullptr)
        dataProtocol = ProtocolGroup::getUdpliteProtocolGroup()->findProtocol(header->getSrcPort());
    callback.dissectPacket(packet, dataProtocol);
    if (packet->getDataLength() > B(0))
        callback.dissectPacket(packet, nullptr);        // padding
    callback.endProtocolDataUnit(&Protocol::udpLite);
}
