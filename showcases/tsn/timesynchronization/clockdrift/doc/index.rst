...
setting the oscillator compensation factor. :ned:`SimpleClockSynchronizer` reads
the actual clock drift rate with an out-of-band mechanism, and sets the
oscillator compensation factor accordingly. With :ned:`Gptp`, the oscillator
compensation factor is set based on the last two time synchronization events.
...
oscillator compensation factor in the synchronized clocks. After that, all
...
events, is used to set the oscillator compensation factor.
...
compensated for, by setting the oscillator compensation factor in clocks. We can
...
.. note:: - When configuring the SimpleClockSynchronizer with a :par:`synchronizationClockTimeError` of 0, the synchronized time perfectly matches the reference.
   - When configuring the SimpleClockSynchronizer with a :par:`synchronizationOscillatorCompensationFactorError` of 1, the compensated clock drift rate perfectly matches the reference.
...
by a 10us period (so they alternate being open). Here is the rest of the gating
...
delay is lower and bounded. The following chart shows the same data zoomed in:
...
to wait in the queue for one or two gate cycles.
