...
In network nodes, protocols often communicate by passing packets along with
meta-information called tags. These tags can be attached to the entire packet,
known as packet tags, or to specific sections of the packet, known as region
tags.

Tags are implemented as very simple C++ classes usually generated by the OMNeT++
MSG compiler. Tags come in three flavors:
...
For example, a request specifying the source and destination MAC address could
be implemented like:
...
The most important packet tag is the :cpp:`PacketProtocolTag`, which specifies
the outermost protocol of the packet. This tag must always be set as it cannot
be automatically identified just by looking at the raw data.

Examples:

 - :cpp:`PacketProtcolTag` specifies the protocol of the packet contents
 - :cpp:`DispatchProtocolReq` specifies the receiver protocol module
 - :cpp:`EncapsulationProcotolReq` specifies the protocol header encapsulation order 
 - :cpp:`SocketReq` specifies the application socket
 - :cpp:`L4PortReq` specifies the sender and receiver port
 ...
 - :cpp:`StreamReq` specifies the TSN stream identifier
 ...
 - :cpp:`SignalPowerReq` specifies send and receive signal power

Many of these tags have their indication counterpart: :cpp:`SocketInd`, :cpp:`InterfaceInd`, etc.

The following code fragment demonstrates how to set packet tags:
...
# TODO: explain Message (Requests, Indications)

Within a network node, protocols interact with one another by sending packets.
INET is very flexible in terms of how protocols can be connected. Protocols can
be connected directly to each other, or they can be connected through one or
more :ned:`MessageDispatcher` modules. This flexibility allows for the creation
of both simple and complex network node architectures.

# TODO: show example UDP, IP, Ethernet
# TODO: show example UDP, dp, IPv4, IPv6

# TODO: add section titles: to avoid big wall of text
# TODO: section title: how to connect modules
# TODO: section title: protocol constants, registerProtocol
...
using message dispatcher modules. More complex network nodes can be created by
grouping protocols into layers and connecting them through :ned:`MessageDispatcher`
modules, which facilitates many-to-one and many-to-many relationships among the
protocols of the layers. It's also possible to use message dispatcher modules
hierarchically within multiple levels of nested compound modules. One could
even connect all protocols to a single central message dispatcher module. In
order to ensure proper functionality, only one instance of a given protocol
module can be connected to a message dispatcher.

To support the above dispatching mechanism in C++ code, certain additional
requirements must be met. Protocol modules must call the :fun:`registerProtocol`
function from the :fun:`initialize` method to inform connected :ned:`MessageDispatcher`
modules of their presence. This allows the dispatcher modules to learn which
gates the protocol modules are connected to. The same protocol is not allowed to
be registered in the same message dispatcher using different gates, because that
would make the dispatching mechanism ambiguous. Additionally, packets and messages
must have the :cpp:`DispatchProtocolReq` tag attached to them in order for the
message dispatcher modules to properly direct them to the intended recipient
within the network node.

The following example shows how a MAC protocol could send up a packet to the
designated :ned:`Ipv4` protocol module without actually knowing where that
module is connected in the network node architecture:
...
terminology, can be one of :cpp:`SP_REQUEST`, :cpp:`SP_INDICATION`,
:cpp:`SP_RESPONSE`, or :cpp:`SP_CONFIRM`. Currently, INET modules only use the
:cpp:`SP_REQUEST` and :cpp:`SP_INDICATION` service primitives, the other two
are only present for completness. The request service primitive is used when a
higher layer protocol module (e.g. :ned:`Tcp`) wants to deliver a packet to a
lower layer protocol module (e.g. :ned:`Ipv4`). Similarly, the indication service
primitive is used when a lower layer protocol module (e.g. :ned:`Ethernet`) wants
to deliver a packet to a higher layer protocol module (e.g. :ned:`Ipv4`).

# TODO use bullet list


A protocol module has several options for determining which protocol to send a
packet to next. # TODO rewrite previous one without the: to send a packet to next! 

The simplest way is to have the intended protocol hardcoded in C++,
like in the case of the :cpp:`UdpSocket` and other protocol-specific sockets.
Another way is to have it specified by the user through a module parameter, as
in the case of network interface modules specifying their expected protocol.
The :ned:`Ipv4` module uses this information to dispatch a packet to the protocol
of the selected route's network interface. 

# TODO The following it is ambiguous
It can also be based on some other
module state, like in the case of TSN stream encoding.

A more flexible approach is to determine the intended protocol based on packet

# TODO the following example is too convoluted

data. For example, the :ned:`Ipv4` module uses the IP protocol ID header field
from the :cpp:`Ipv4Header` to look up the next protocol to dispatch to in the

# TODO code examples

    auto id = ipv4Header->getProtocolId();
    auto p = getIpProtocolGroup()->getProtocol(id);
    packet->addTag<DispatchProtocolReq>()->setProtocol(p);


IP protocol group for received packets. Additionally, the dispatch protocol can
often be determined by some packet meta-data. For example, the :ned:`Tcp` module
uses the type of the destination address from the :cpp:`L3AddressReq` to determine
if the packet should be sent to the :ned:`Ipv4` or :ned:`Ipv6` module. Additionally,
the indended dispatch protocol can be determined by the other modules attaching
an :cpp:`EncapsulationProtocolReq` to the packet in an earlier stage of the
packet processing.

# TODO how a typical protocol uses the EncapsulationProtocolReq to set the DispatchProtocolReq

    auto req = packet->getTag<EncapsulationProtocolReq>();
    auto p = req->getProtocol(0);
    if (p == nullptr)
        p = &Protocol::mac;
    packet->addTag<DispatchProtocolReq>()->setProtocol(p);
...
A packet typically includes multiple protocol-specific headers, such as TCP, IP,
Ethernet, and sometimes additional optional headers like 802.1Q, 802.1R,
802.1AE, etc. The arrangement of these headers is determined by the encapsulation
process applied to the packet, which is dependent on the order in which the
packet reaches the relevant protocol modules.

The above encapsulation process can be different for each packet and it often
depends on various policies configured by the user. For instance, an application might need to send a specific packet on a specific VLAN.
In this scenario, the application would attach a VLAN identifier as meta-data
to the packet. However, it cannot immediately request for the packet to be sent
to the relevant 802.1Q protocol module, as the packet needs to first be delivered
to the UDP and IP protocol. To achieve the desired protocol encapsulation order,
the application would also attach an :cpp:`EncapsulationProtocolReq` specifying
that the packet should ultimately be delivered to the 802.1Q protocol for
encapsulation. The underlying protocol modules will use this information to
determine when the 802.1Q encapsulation should take place.
...
meta-data attached to the packet acts as parameters for the requested processing
steps.

# TODO too complicated, encapsulation request may change during processing, for example... 
The encapsulation request may be changed during earlier stages of packet
processing, it may be ignored, but it is generally considered.

# TODO default forwarding policy
